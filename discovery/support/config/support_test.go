...
func TestMSPIDMapping(t *testing.T) {
	...
	cryptogen, err := gexec.Build(filepath.Join("github.com", "hyperledger", "mchain", "common", "tools", "cryptogen"))
	assert.NoError(t, err)
	defer os.Remove(cryptogen)

	idemixgen, err := gexec.Build(filepath.Join("github.com", "hyperledger", "mchain", "common", "tools", "idemixgen"))
	assert.NoError(t, err)
