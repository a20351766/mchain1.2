...
Shared configuration for a Hyperledger Mchain blockchain network is
...
``mchain/protos/common/configtx.proto``. The Envelope of type
...
in ``mchain/protos/peer/configuration.proto``. The notations
``common.<MSG>``, ``msp.<MSG>``, and ``orderer.<MSG>`` correspond
similarly, but with their messages defined in
``mchain/protos/common/configuration.proto``,
``mchain/protos/msp/mspconfig.proto``, and
``mchain/protos/orderer/configuration.proto`` respectively.
