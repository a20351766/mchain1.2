...
other elements in a Hyperledger Mchain network.
...
Hyperledger Mchain implements smart contracts with a technology concept it calls
...
a deliberate redundancy in a Mchain network --- it avoids single points of
...
Hyperledger Mchain network. When a peer is first created, it has
...
We'll see a little later why the concept of **channels** in Hyperledger Mchain
...
help you get started with Hyperledger Mchain, but don't worry --- what's most
...
chaincodes. The Hyperledger Mchain Software Development Kit (SDK) makes this
...
This strict ordering of transactions within blocks makes Hyperledger Mchain a
little different from other blockchains where the same transaction can be
packaged into multiple different blocks. In Hyperledger Mchain, this cannot
...
in the ledger is immutably assured. Hyperledger Mchain's finality means that a
...
Hyperledger Mchain --- all transactions are marshalled into a strict order ---
...
they relate to in Hyperledger Mchain. We've seen that peers are in many ways the
