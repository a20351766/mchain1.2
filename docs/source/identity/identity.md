...
A digital identity furthermore has some additional attributes that Mchain uses
...
(MSP) is how this is achieved in Mchain. More specifically, an MSP is a component
that defines the rules that govern the valid identities for this organization.
The default MSP implementation in Mchain uses X.509 certificates as identities,
...
permissioned blockchain system (like Mchain). For example, you'll see that
...
It's because CAs are so important that Mchain provides a built-in CA component to
allow you to create CAs in the blockchain networks you form. This component --- known
as **Mchain CA** is a private root CA provider capable of managing digital identities of
Mchain participants that have the form of X.509 certificates.
Because Mchain CA is a custom CA targeting the Root CA needs of Mchain,
...
(even in a test environment), Mchain CA can be used to provide and manage
...
If you're interested, you can read a lot more about Mchain CA
[in the CA documentation section](http://hyperledger-mchain-ca.readthedocs.io/).
...
valid certificate. For more in-depth information about CRLs, click [here](https://hyperledger-mchain-ca.readthedocs.io/en/latest/users-guide.html#generating-a-crl-certificate-revocation-list).
